...
async def roll(ctx, dice: str = None):
    ...
    if not dice:  # Same as 1d20
        ...
        if diceRollOne >= diceRollTwo:
            await ctx.send('>>> ' + ctx.author.mention + ' Total: ' + str(diceRollOne) + ' Rolls: ' + str(
                diceRollOne) + ', ' + str(diceRollTwo))
        else:
            await ctx.send('>>> ' + ctx.author.mention + ' Total: ' + str(diceRollTwo) + ' Rolls: ' + str(
                diceRollOne) + ', ' + str(diceRollTwo))
        return
    ...
    try:
        ...
        await ctx.send(
            '>>> ' + ctx.author.mention + ' Use #d# formating. For more information, use the ' + PREFIX + 'help command')
        return
...
async def fate(ctx, dice: str = None):
    ...
    try:
        ...
        await ctx.send(
            '>>> ' + ctx.author.mention + ' Use a whole number. For more information, us the ' + PREFIX + 'help command')
        return
...
async def chad(ctx, dice: str = None):
    total = 0
    diceResultParse = [[1, 1, 4, 4], [1, 1, 2, 5, 6, 6], [1, 1, 2, 3, 6, 7, 8, 8], [1, 1, 2, 2, 3, 8, 9, 9, 10, 10],
    ...
    try:
        ...
        if '+' in dice:
            ...
            if int(diceSplit[0]) > MAX_ROLL or int(diceSplit[1]) > MAX_ROLL or int(diceSplit[2]) > MAX_MOD:
                await ctx.send('>>> ' + ctx.author.mention + ' Please use a smaller number')
                return
            if int(diceSplit[1]) != 4 and int(diceSplit[1]) != 6 and int(diceSplit[1]) != 8 and int(
                    diceSplit[1]) != 10 and int(diceSplit[1]) != 12 and int(diceSplit[1]) != 20:
